...
(function () {
  ...
  // --------------------------------------------------------------- lights --
  const hemi = new THREE.HemisphereLight(0x2c3a5c, 0x07080e, 0.55);
  scene.add(hemi);
  ...
  // static circle colliders (planters, lamps, vents, exit posts) — resolved on
  // the XZ plane for the player and ground enemies so nothing clips through props
  const colliders = [];
  function collideXZ(pos, radius) {
    let hit = false;
  ...
  for (let i = 0; i < 8; i++) {
    ...
    world.add(g);
    colliders.push({ x: g.position.x, z: g.position.z, r: 0.3 });
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    colliders.push({ x: p.position.x, z: p.position.z, r: 1.6 });
  }
  ...
  const steamVents = [];
  {
    ...
    for (const [r, a] of spots) {
      ...
      colliders.push({ x: ox, z: oz, r: 0.85 });
  ...
  // dawn rig — hidden until the escape: a rising sun beyond the exit, warm key
  // light and a fog shift from midnight blue to first light
  const dawn = (() => {
    const tex = canvasTex(128, 128, (g, w, h) => {
      const gr = g.createRadialGradient(64, 64, 6, 64, 64, 64);
      gr.addColorStop(0, 'rgba(255,244,214,1)');
      gr.addColorStop(0.3, 'rgba(255,206,130,0.95)');
      gr.addColorStop(0.7, 'rgba(255,150,90,0.4)');
      gr.addColorStop(1, 'rgba(255,140,80,0)');
      g.fillStyle = gr; g.fillRect(0, 0, w, h);
    });
    const sun = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true, opacity: 0, depthWrite: false, depthTest: false }));
    sun.position.set(0, 6, -82); sun.scale.setScalar(30); sun.renderOrder = 999; scene.add(sun);
    const light = new THREE.DirectionalLight(0xffc37a, 0);
    light.position.set(0, 24, -90); scene.add(light);
    return { sun, light, fogFrom: new THREE.Color(0x0a0e1c), fogTo: new THREE.Color(0x55384a) };
  })();
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (game.mode === 'escape') game.escapeT = Math.min(game.escapeT, 0.3);
  ...
  // ---- THE EXIT — the way out of the rat race, revealed when the boss falls ----
  let exit = null;
  function openExit() {
    const g = new THREE.Group();
    const gold = new THREE.MeshStandardMaterial({ color: 0xc9a44a, roughness: 0.3, metalness: 0.9, emissive: 0x6b4d12, emissiveIntensity: 0.5 });
    const postGeo = new THREE.BoxGeometry(0.45, 5.2, 0.45);
    const postL = new THREE.Mesh(postGeo, gold); postL.position.set(-1.6, 2.6, 0); postL.castShadow = true; g.add(postL);
    const postR = postL.clone(); postR.position.x = 1.6; g.add(postR);
    const lintel = new THREE.Mesh(new THREE.BoxGeometry(3.65, 0.45, 0.45), gold);
    lintel.position.y = 5.0; g.add(lintel);
    const sign = new THREE.Mesh(
      new THREE.PlaneGeometry(3.4, 0.85),
      new THREE.MeshBasicMaterial({ map: signTex('EXIT →', '#7ff0b8'), transparent: false })
    );
    sign.position.set(0, 5.85, 0.1); g.add(sign);
    const portalMat = new THREE.MeshBasicMaterial({
      map: canvasTex(64, 128, (gg, w, h) => {
        const gr = gg.createLinearGradient(0, 0, 0, h);
        gr.addColorStop(0, 'rgba(255,236,180,0.95)');
        gr.addColorStop(0.6, 'rgba(255,204,110,0.8)');
        gr.addColorStop(1, 'rgba(255,170,80,0.55)');
        gg.fillStyle = gr; gg.fillRect(0, 0, w, h);
      }),
      transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide,
    });
    const portal = new THREE.Mesh(new THREE.PlaneGeometry(2.75, 4.9), portalMat);
    portal.position.y = 2.55; g.add(portal);
    const glow = new THREE.PointLight(0xffd98a, 1.7, 34, 1.6);
    glow.position.set(0, 3, 1.5); g.add(glow);
    g.position.set(0, 0, -50);
    scene.add(g);
    const cols = [{ x: -1.6, z: -50, r: 0.55 }, { x: 1.6, z: -50, r: 0.55 }];
    colliders.push(...cols);
    exit = { g, cols, portalMat, t: 0 };
    spawnBeam(0, -50, 0xffd98a);
    showAnnounce('THE EXIT HAS OPENED', 'LEAVE THE RAT RACE — FOLLOW THE GOLDEN LIGHT', 3800);
    Audio.sfx.perk();
    return exit;
  }
  function removeExit() {
  ...
  function startEscape() {
    ...
    hideEdgeArrows();
    for (const h of hazards) h.life = 0;
    heroRig.briefcase.visible = false;
    Audio.sfx.fanfare();
  }
  ...
  function updateEdgeArrows() {
    let used = 0;
    const list = [];
    if (exit) list.push({ mesh: exit.g, kind: 'exit' });
    if (boss) list.push(boss);
    for (const e of enemies) if (e !== boss) list.push(e);
    for (const e of list) {
      ...
      d.classList.toggle('exit', e.kind === 'exit');
  ...
  function resetGame() {
    ...
    removeExit();
    dawn.sun.material.opacity = 0;
    dawn.light.intensity = 0;
    hemi.intensity = 0.55;
    scene.fog.color.copy(dawn.fogFrom);
  ...
  function killEnemy(e) {
    ...
    if (e.kind === 'boss') {
      ...
      Audio.setIntense(false);
      if (!game.overtime) { openExit(); return; }
    }
  }
  ...
  function updatePlayer(dt, rawDt) {
    ...
    // solid props
    collideXZ(player.pos, 0.45);
    // reaching the open exit ends the night
    if (exit && Math.hypot(player.pos.x - exit.g.position.x, player.pos.z - exit.g.position.z) < 1.7) {
      startEscape();
      return;
    }
  ...
  function updateEnemies(dt) {
    for (const e of [...enemies]) {
      ...
      // ground units collide with props; a charging shredder that rams one goes dizzy
      if (e.kind === 'walker' || e.kind === 'copier' || e.kind === 'shredder') {
        const rammed = collideXZ(e.mesh.position, e.r * 0.7);
        if (rammed && e.kind === 'shredder' && e.state === 'charge') {
          e.state = 'dizzy'; e.stateT = 1.2;
          e.eye.scale.set(1, 1, 1);
          burst(e.mesh.position, 0xb9c2d6, 0.16, 12);
          Audio.sfx.hit();
        }
      }
      // decay knockback
  ...
  function updateFx(dt) {
    ...
    if (exit) {
      exit.t += dt;
      exit.portalMat.opacity = 0.72 + Math.sin(exit.t * 3.2) * 0.18;
    }
  }
  ...
  function updateDirector(dt) {
    ...
    } else if (game.spawnQueue.length === 0 && enemies.length === 0 && game.mode === 'playing' && !exit) {
      waveCleared();
    }
    // combo decay
  ...
  function frame() {
    ...
    const rawDt = Math.min(rawDelta, 0.05);
    // cinematic timers track wall time so scripted sequences don't stretch on slow devices
    const wallDt = Math.min(rawDelta, 0.5);
    ...
    if (game.mode === 'playing') {
      ...
      // the way out: Paul walks through the golden door as dawn breaks
      game.escapeT -= wallDt;
      const spd = 2.4;
      player.pos.z -= spd * wallDt;
      player.walkPhase += spd * wallDt * 1.6;
      const swing = Math.sin(player.walkPhase) * 0.55;
      heroRig.legL.hip.rotation.x = swing;
      heroRig.legR.hip.rotation.x = -swing;
      heroRig.legL.knee.rotation.x = Math.max(0, -swing) * 0.8;
      heroRig.legR.knee.rotation.x = Math.max(0, swing) * 0.8;
      heroRig.armL.shoulder.rotation.x = -swing * 0.7;
      heroRig.armR.shoulder.rotation.x = swing * 0.7;
      heroRig.body.rotation.x = 0;
      heroRig.root.position.copy(player.pos);
      heroRig.root.position.y = Math.abs(Math.sin(player.walkPhase)) * 0.05;
      heroRig.root.rotation.y = Math.PI; // facing the exit, away from the city
      // dawn breaks
      const k = clamp(1 - game.escapeT / 5.2, 0, 1);
      dawn.sun.material.opacity = k;
      dawn.sun.scale.setScalar(30 + k * 34);
      dawn.light.intensity = k * 1.3;
      hemi.intensity = 0.55 + k * 0.5;
      scene.fog.color.copy(dawn.fogFrom).lerp(dawn.fogTo, k * 0.85);
      // camera trails dead-centre through the doorway, into the light
      _v1.set(player.pos.x, 2.3, player.pos.z + 8);
      camera.position.lerp(_v1, 1 - Math.pow(0.001, wallDt));
      camera.lookAt(player.pos.x, 1.6, player.pos.z - 8);
      updateFx(wallDt * 0.5);
      if (game.escapeT <= 0) winGame();
    ...
    updateAmbient(rawDt);
    if (game.mode !== 'title' && game.mode !== 'lookdev' && game.mode !== 'cinematic' && game.mode !== 'escape') updateCamera(rawDt);
  ...
  window.__AH = {
    ...
    state() {
      return {
        ...
        hazards: hazards.length, pickups: pickups.length, exit: !!exit,
    ...
    gotoExit() { if (exit) player.pos.set(exit.g.position.x, 0, exit.g.position.z + 1.2); },
    colliders,
    settings: SETTINGS,
  };
})();
